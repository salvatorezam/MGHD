...
def build_round_repetition(layout: dict[str, Any], round_idx: int) -> CudaQKernel:
    ...
    for i, data_q in enumerate(layout["data"][:-1]):
        if i < len(ancillas):
            ...
            cz_ops.append({"gate": "CZ", "qubits": (data_q, ancilla_q), "duration_ns": 40.0})

    if cz_ops:
        kernel.add_layer(cz_ops, "CZ")

    # Layer 2: Measurement of ancillas (Z-basis)
    meas_ops = [{"gate": "MEASURE_Z", "qubits": (q,), "duration_ns": 20.0} for q in ancillas]
...
def build_round_surface(layout: dict[str, Any], round_idx: int) -> CudaQKernel:
    ...
    for layer_idx, cz_layer in enumerate(cz_layers):
        ...
        for q1, q2 in cz_layer:
            if (is_x_round and q1 in active_ancillas) or (not is_x_round and q1 in active_ancillas):
                relevant_czs.append(
                    {
                        "gate": "CZ",
                        "qubits": (q1, q2),
...
def build_round_bb(
    hx: np.ndarray, hz: np.ndarray, mapping: dict[int, int], round_idx: int
) -> CudaQKernel:
    """
    ...
    for check_idx in range(num_checks):
        ...
        data_qubits = [
            mapping.get(q, q) for q in range(num_qubits) if check_matrix[check_idx, q] == 1
        ]
    ...
    if is_x_round:
        ...
        prx_prep_ops = [{"gate": "PRX", "qubits": (q,), "duration_ns": 20.0} for q in ancillas]
    ...
    if is_x_round:
        ancillas = [ancilla_start + i for i in range(num_checks)]
        prx_meas_ops = [{"gate": "PRX", "qubits": (q,), "duration_ns": 20.0} for q in ancillas]
...
def analyze_idle_qubits(
    kernel: CudaQKernel, total_qubits: int
...
def optimize_layout_for_device(
    code_distance: int,
    device_couplers: list[tuple[int, int]],
    coupler_fidelities: dict[tuple[int, int], float],
) -> dict[str, Any]:
    """
