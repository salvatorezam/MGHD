...
class GarnetFoundationPriors:
    ...
    def sample_pseudo_device(
        self, rng: np.random.Generator, n_qubits: int = 20
        ...
        eps0, eps1 = {}, {}
        for q in range(n_qubits):
            eps0[q] = np.clip(eps0_base + rng.uniform(-jitter["eps"], jitter["eps"]), 0.005, 0.1)
            eps1[q] = np.clip(eps1_base + rng.uniform(-jitter["eps"], jitter["eps"]), 0.005, 0.1)
