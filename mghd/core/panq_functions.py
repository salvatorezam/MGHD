...
def surface_code_edges(code):
    ...
    half = int(len(dst_ids) / 2)
    dst_ids = dst_ids - 1
    dst_ids[half:] = dst_ids[half:] + code.d ** 2
...
def generate_syndrome_error_volume(code, error_model, p, batch_size, for_training=True,
    ...
    if backend == "cudaq":
        ...
        # Use CUDA-Q sampler
        batch = sample_surface_cudaq(
            mode=cudaq_mode,
            batch_size=batch_size, 
            T=cudaq_cfg.get("T", 1),
            layout=cudaq_cfg["layout"],
...
def logical_error_rate(gnn, testloader, code, enable_osd=False, osd_decoder=None):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, targets, src_ids, dst_ids) in enumerate(testloader):
            ...
            logical_mask = np.any(code.logical_errors(rf) != 0, axis=1)
            n_l_error += logical_mask.sum()

            n_total_ler += np.logical_or(logical_mask, mse).sum()
            n_test += batch_size
...
def save_model(model, filename, confirm=True):
    if confirm:
        try:
            ...
            raise Exception('The notebook should be run or validated with skip_training=True.') from exc
