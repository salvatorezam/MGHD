...
class CropShardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert to tensors
        toT = lambda x, dtype=None: torch.as_tensor(x if not isinstance(x, torch.Tensor) else x, dtype=dtype)
        packed = PackedCrop(
            x_nodes=toT(item["x_nodes"], torch.float32),
            node_mask=toT(item["node_mask"], torch.bool),
            node_type=toT(item["node_type"], torch.int8),
            edge_index=toT(item["edge_index"], torch.long),
            edge_attr=toT(item["edge_attr"], torch.float32),
            edge_mask=toT(item["edge_mask"], torch.bool),
            seq_idx=toT(item["seq_idx"], torch.long),
            seq_mask=toT(item["seq_mask"], torch.bool),
            g_token=toT(item["g_token"], torch.float32),
            y_bits=toT(item["y_bits"], torch.int8),
            meta=item["meta"],
...
def make_bucket_sampler(ds: CropShardDataset, *, stage="stage1", seed=0):
    ...
    keys = list(buckets.keys()); keys.sort()
    heads = {k:0 for k in keys}
    while True:
        progressed = False
        for k in keys:
            lst = buckets[k]; h = heads[k]
            if h < len(lst):
                order.append(lst[h]); heads[k] = h+1; progressed = True
        if not progressed: break
    rng.shuffle(order)
    class _S(Sampler):
        def __iter__(self): return iter(order)
        def __len__(self): return len(order)
    return _S()

def collate_packed(batch):
    # Single-sample per item; batch via stacking on new dim for tensors that allow it.
    # For simplicity and to keep compatibility with existing microbatching, keep batch_size as micro-accumulation.
    return batch

def bce_binary_head_loss(logits: torch.Tensor, node_mask: torch.Tensor, node_type: torch.Tensor, y_bits: torch.Tensor, *, sample_weight: torch.Tensor | None = None) -> torch.Tensor:
    ...
    if sample_weight is not None:
        w = sample_weight.to(logits.device).clamp_min(0.5).clamp_max(3.0)
        loss_all = loss_all * w
    return loss_all.mean()

def parity_auxiliary_loss(logits: torch.Tensor, node_mask: torch.Tensor, node_type: torch.Tensor, H_sub: np.ndarray) -> torch.Tensor:
...
def projection_aware_logits_to_bits(logits: torch.Tensor, projector_kwargs: dict[str, any], *, data_mask: torch.Tensor) -> np.ndarray:
    ...
    if hasattr(cc, "ml_parity_project"):
        ...
        s_sub = projector_kwargs.get("s_sub", np.zeros(H_sub.shape[0], dtype=np.uint8) if H_sub is not None else np.array([], dtype=np.uint8))
...
def train_inprocess(ns) -> str:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for idx in sampler:
            ...
            loss_bce = bce_binary_head_loss(logits, packed.node_mask.to(device), packed.node_type.to(device), packed.y_bits.to(device), sample_weight=sample_weight)
            # Parity aux
            loss_par = args.parity_lambda * parity_auxiliary_loss(
                logits, packed.node_mask.to(device), packed.node_type.to(device),
                H_sub=packed.H_sub
            )
            ...
            if args.projection_aware:
                ...
                proj_bits = projection_aware_logits_to_bits(
                    logits, projector_kwargs={"H_sub": packed.H_sub, "side": getattr(packed.meta, "side", "Z")}, data_mask=data_mask
                )
                with torch.no_grad():
                    # Extract target bits for data qubits only
                    mask_data = (packed.node_type.to(device) == 0) & packed.node_mask.to(device)
                    target_full = (packed.y_bits.cpu().numpy().clip(0,1)).astype(np.uint8)
                ...
                # Self-consistency: penalize flips introduced by projection vs raw threshold on data-qubits
        ...
        if avg < best_loss:
            best_loss = avg
            torch.save({"model":model.state_dict(),"epoch":epoch,"loss":avg}, os.path.join(args.save,"best.pt"))
