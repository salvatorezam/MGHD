...
**Important note:** 
1) Always activate mlqec-env while running a file/command in the terminal otherwise the necessary packages cannot be used.
2) Whenever new tests or temporary files are created they must be deleted after use immediately to lessen the clutter and keep the repository clean. 
...
- Always `conda activate mlqec-env`.
- Always run from workdir: `cd /u/home/kulp/MGHD/scratchpad/initial-test` before invoking scripts.
- Current runs launched with `--teacher mwpf` per policy; post‑eval watchers attached.
...
## Model/Graph Conventions
- MGHD authoritative graph indices now follow Z‑first‑then‑X check ordering to exactly match the syndrome layout above. If you provide raw `s_bin` in `[B,8]` (Z then X), the first 4 features map to Z checks and the next 4 to X checks. This resolves a prior mismatch where X checks were placed before Z checks in the internal graph.

## Teachers & Labels
- **Default teacher (d=3)**: MWPM (PyMatching) for stability; LUT is also available and aligned. MWPF via Sinter DEM can be used when fully validated (see Implementation Summary notes).
- **Other options**: MWPF (circuit‑level matching), LUT (fastpath), or ensemble policy.
...
- **MGHD (Mamba + GNN)**: `poc_my_models.py`
  ...
  - Attention: channel attention (SE) is REQUIRED and always enabled in training and evaluation; evaluator mirrors this.
  - Stabilization: optional post‑Mamba LayerNorm can be enabled via `mamba_params['post_mamba_ln']=True`.
...
### 2) Foundation Model (formerly "Step‑11")
   ...
   - **Loss**: BCEWithLogits (bit‑head); optional parity‑aware auxiliary loss (differentiable XOR expectation on Hx/Hz parities) controlled by `--parity-lambda` (FP32 outside autocast). Grad clip default=1.0.
   - **Schedule**: cosine with warmup; curriculum over p ∈ {0.02, 0.03, 0.05, 0.08}; 20–40 epochs; early stop/selection by coset‑validated val LER. For specialization at p=0.05, use `--train-p 0.05` and increase `--val-N`.
   - **EMA**: enable EMA weights with `--ema-decay` (e.g., 0.999); validation uses EMA snapshot to stabilize selection.
   - **Profiles**: S/M/L/XL (parameterized); select by LER ± CI under latency budget. XL (a.k.a. L+) uses n_iters=10 and wider Mamba/head.
   - **Overrides (S tuning)**: `--ov-n-iters`, `--ov-node-feats`, `--ov-edge-feats`, `--ov-msg-size`, `--ov-msg-drop`, `--ov-gru-drop`, `--ov-mamba-d-model`, `--ov-mamba-d-state`, `--ov-mamba-expand`, `--post-mamba-ln`.

### 3) Student Model
   - Initialize from foundation checkpoint; fine‑tune on exact Garnet calibrations (no deltas) under same loss/validation gates.

### 4) Optuna Sweeps
   - **Objective**: minimize forward‑path LER at p=0.05 after a short run (defaults: 6 epochs × 800 steps, B=512).
   - **Tooling**: `tools/sweep_s_optuna.py` (S profile by default) — CUDA‑Q sampler + MWPF labels; channel attention on; manual label smoothing inside BCE.
   - **Promote** best trials to 20‑epoch confirmatory runs (fix `--train-p 0.05`, `--val-N 4096`, EMA on, parity loss on), then run N=10k per‑p forward eval and latency.

## Evaluation & Reporting
- **LER harness**: `tools/eval_ler.py` with `--decoder {mghd_forward,mghd,mwpm,mwpf,relay,fastpath}`; parity/coset enforced; Wilson CIs (95%); N≥10k per‑p.
  - Use `mghd_forward` to mirror the training forward path (including channel attention) and avoid decode path mismatches.
...
- **Auto post‑eval**: `tools/auto_post_eval.py` watches a run outdir (by PID) and writes forward‑path N=10k per‑p results on completion.
...
- **Practical note**: Recent sweeps indicate S with tuned parameters (e.g., n_iters≈8, d_model≈192, d_state≈80, edges≈256, small msg head, LR≈6e‑5, WD≈7e‑5) can beat MWPF at p=0.05 while offering better latency than L/XL.
