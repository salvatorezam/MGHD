...
def main():
    ...
    # Verify real CUDA-Q backend (no fallback)
    try:
        import cudaq
        ...
        try:
            ...
            print("Execution Target: default")
        ...
        print("✗ CUDA-Q import failed - no real backend available")
        sys.exit(1)
    ...
    try:
        from cudaq_backend.circuits import CudaQKernel
        try:
            _smoke = CudaQKernel("smoke", [])
            print("✓ Real CUDA-Q kernel class available")
        except Exception as e:
            print(f"Warning: Kernel smoke test failed: {e}")
    except Exception as e:
        if "fallback" in str(e).lower() or "mock" in str(e).lower():
            print(f"✗ Fallback mode detected: {e}")
            sys.exit(1)
        else:
            print(f"Warning: Kernel test failed but no fallback detected: {e}")
