...
from panq_functions import GNNDecoder, collate, fraction_of_solved_puzzles, compute_accuracy, logical_error_rate, \
    surface_code_edges, generate_syndrome_error_volume, adapt_trainset, ler_loss,save_model,load_model,ler_loss_itr,calc_test_losses
...
print("loss = msx hxperp + sloss + eloss with 2 bit syndrome")
...
gnn = GNNDecoder(dist=dist, n_node_inputs=n_node_inputs, n_node_outputs=n_node_outputs, n_iters=n_iters,
                 n_node_features=n_node_features, n_edge_features=n_edge_features, msg_net_size=msg_net_size, msg_net_dropout_p=msg_net_dropout_p, gru_dropout_p=gru_dropout_p)
gnn.to(device)
...
fnameload = f"trained_models/d{d}_{error_model_name}_30_500_500_200000_0.15_10000_0.05_0.0001_0.0001_gnn.pth 0.0048_0.0047 "
model_loaded = False
...
"""
    Train
"""
fnamenew = f"trained_models/d{dist}_{error_model_name}_{n_iters}_{n_node_features}_{n_edge_features}_{len_train_set}_{max_train_err_rate}_{len_test_set}_{test_err_rate}_{lr}_{weight_decay}_"
if model_loaded:
    fnamenew = f"trained_models/d{dist}_from_d{d}_{error_model_name}_{n_iters}_{n_node_features}_{n_edge_features}_{len_train_set}_{max_train_err_rate}_{len_test_set}_{test_err_rate}_{lr}_{weight_decay}_"
...
# torch.cuda.nvtx.range_push("train data generate")
...
for epoch in range(epochs):
    ...
    trainset = adapt_trainset(generate_syndrome_error_volume(code, error_model, p=max_train_err_rate, batch_size=len_train_set),
                              code, num_classes=n_node_inputs)
    trainloader = DataLoader(trainset, batch_size=batch_size, collate_fn=collate, shuffle=False,num_workers=4)
    torch.cuda.nvtx.range_pop()
    epoch_loss = []
    for i, (inputs, targets, src_ids, dst_ids) in enumerate(trainloader):
        torch.cuda.nvtx.range_push(f"batch_{i}")
        torch.cuda.nvtx.range_push(f"data_to_gpu")
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
            ...
            torch.cuda.nvtx.range_pop()
            torch.cuda.nvtx.range_push(f"batch_losses")
            torch.cuda.nvtx.range_push(f"ler_loss_itr")
            ler_loss_all = ler_loss_itr(outputs, targets, code)
            # celoss_itr = criterion(outputs, targets.unsqueeze(0).repeat(n_iters, 1))
            torch.cuda.nvtx.range_pop()
            for j, out in enumerate(outputs):
                # torch.cuda.nvtx.range_push(f"eloss")
                # eloss = criterion(out.view(-1, size, n_node_inputs)[:, error_index:].reshape(-1, n_node_inputs),
                #                   targets.view(-1, size)[:, error_index:].flatten())
                # torch.cuda.nvtx.range_pop()
                # torch.cuda.nvtx.range_push(f"sloss")
                # sloss = criterion(out.view(-1, size, n_node_inputs)[:, :error_index].reshape(-1, n_node_inputs),
                #                   targets.view(-1, size)[:, :error_index].flatten())
                # torch.cuda.nvtx.range_pop()
                torch.cuda.nvtx.range_push(f"ce_loss")
                celoss = criterion(out, targets)
                torch.cuda.nvtx.range_pop()
                # torch.cuda.nvtx.range_push(f"ler_loss")
        ...
        # torch.cuda.nvtx.range_pop(f"epoch_{epoch}_batch_{i}")
        torch.cuda.nvtx.range_pop()
    epoch_loss = torch.mean(torch.tensor(epoch_loss)).item()
    torch.cuda.nvtx.range_push(f"epoch_{epoch}_test_losses")
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        # torch.cuda.nvtx.range_push(f"fraction_solved")
        # fraction_solved = fraction_of_solved_puzzles(gnn, testloader, code)
        # torch.cuda.nvtx.range_pop()
        # torch.cuda.nvtx.range_push(f"test_loss")
        # test_loss = compute_accuracy(gnn, testloader, code)
        # torch.cuda.nvtx.range_pop()
        # torch.cuda.nvtx.range_push(f"ler_calc")
        # lerx, lerz, ler_tot = logical_error_rate(gnn, testloader, code)
        # torch.cuda.nvtx.range_pop()

        """ new ..one forward pass for inference"""
        torch.cuda.nvtx.range_push(f"calc_test_losses")
        fraction_solved, test_loss, lerx, lerz, ler_tot = calc_test_losses(gnn, testloader, code)
        torch.cuda.nvtx.range_pop()
    torch.cuda.nvtx.range_pop()
    scheduler.step()  # update lr
    # print(optimizer.param_groups[0]['lr'],optimizer.param_groups[0]["weight_decay"])
    # torch.cuda.nvtx.range_pop(f"epoch_{epoch}")
    torch.cuda.nvtx.range_pop()
    # repoch.append(epoch)
    ...
    if ler_tot < min_test_err_rate:
        ...
        min_lerz = lerz
        # tools.save_model(gnn, fnamenew + f'gnn.pth {lerx}_{lerz}_{ler_tot} {epoch}', confirm=False)
