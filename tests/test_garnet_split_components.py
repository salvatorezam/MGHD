...
def test_split_components_returns_expected_keys():
    ...
    comps_z = split_components_for_side(side="Z", Hx=Hx, Hz=Hz, synZ=synZ, synX=synX, coords_q=coords_q, coords_c=coords_c)
    comps_x = split_components_for_side(side="X", Hx=Hx, Hz=Hz, synZ=synZ, synX=synX, coords_q=coords_q, coords_c=coords_c)
    for comps in (comps_z, comps_x):
        assert isinstance(comps, list)
        if comps:
            c = comps[0]
            for key in ("H_sub", "xy_qubit", "xy_check", "synd_bits", "bbox_xywh", "k", "r", "kappa_stats", "qubit_indices", "check_indices"):
                assert key in c
